...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listDublicate = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {
            String s = list.get(i);
            char[] numbers = s.toCharArray();
            if ((numbers.length % 2) <= 0)
                listDublicate.add(doubleString(s));
            else
            {
                listDublicate.add(tripleString(s));
            }
        }

        for (int i = 0; i < listDublicate.size(); i++)
        {
            System.out.println(listDublicate.get(i));
        }
    }
    ...
    public static String tripleString(String s)
    {
        String string = s + " " + s + " " + s;
        return string;
    }
